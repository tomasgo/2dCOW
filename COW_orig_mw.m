function [Warping,XWarped,Diagnos] = cow_orig_mw(T,X,Seg,Slack,Options)
...
XInd = repmat({':'},length(dimX) - 2);
XRep = cell(length(dimX) - 2,1);
tic
for i_dim = 3:length(dimX)
   XRep{i_dim} = ones(dimX(i_dim),1);
end
...
for i_seg = 1:nSeg                             % Loop over segments
   ...
   dimX_Seg      = [dimX(1),(bP(i_seg + 1) - bP(i_seg) + 1) * prod(dimX(3:end))];
   XSeg          = reshape(X(:,:,bP(i_seg):bP(i_seg + 1)),dimX_Seg);  % Segments i_seg of signals X
   ...
   for i_node = Node_A:Node_Z                                  % Loop over nodes (i.e. possible boundary positions) for segment i_seg
      ...
      if N_AA % Sometimes boundaries are ineffective and few nodes are allowed that cannot be reached
         ...
         Ti_Seg                        = reshape((Ti_Seg + Coeff_b .* Ti_diff)',c * prod(dimX(3:end)),N_AA);    % Interpolate for all allowed predecessors
