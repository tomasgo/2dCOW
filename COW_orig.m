function [Warping,XWarped,Diagnos] = cow_orig(T,X,Seg,Slack,Options)
...
% Giorgio Tomasi / Frans van den Berg 060912 (GT)
%
% Thomas Skov 061912 - line 277 changed to work in MATLAB version 6.5
%
...
if Pred_Bound 
   ...
   if not(isequal(Seg(:,1),ones(2,1)) & isequal(Seg(:,end),[pT,pX]'))
      error('End points must be equal to 1 and to the length of the pattern/target'); end
   
   LenSeg = diff(Seg,1,2);    % LenSeg(1,:): Length of the segments in the - 1
   if not(all(LenSeg >= 2))
      error('Segments must contain at least two points'); end
   ...
   if Seg > min(pX,pT)
      error('Segment length is larger than length of the signal'); end
   ...
   if Options(3) % Segments in the signals can have different length from those in the target
      ...
      if floor((pX - 1) / (Seg - 1)) ~= nSeg
         error('For fixed segment lengths the target and the signal do not have the same number of segments'); end
   ...
   if temp > 0
      ...
      if Options(1)
         fprintf('\n Segments: %i points x %i segments (target)',LenSeg(2,1) + 1,nSeg); end
   end
   ...
   if temp > 0
      ...
      if Options(1)
         fprintf('\n           %i points x %i segments (signals)\n',LenSeg(2,1) + 1,nSeg); end
   end

end
if any(LenSeg(:) <= Slack + 1) % Two points are the minimum required for linear interpolation
   error('The slack cannot be larger than the length of the segments'); end

bT      = cumsum([1,LenSeg(1,:)]);
bP      = cumsum([1,LenSeg(2,:)]);
Warping = zeros(nX,nSeg + 1,2);
...
Bounds_a    = bT(ones(2,1),1:nSeg + 1) + offs;
Bounds_b    = bT(ones(2,1),1:nSeg + 1) + offs(:,nSeg + 1:-1:1);
...
if Options(4)
   ...
   if abs(pT - pX) > Options(4)
      error('The band is too narrow and proper correction is not possible');end

   Bounds(1,:) = max(Bounds(1,:),max(0,pT/pX * bP - Options(4)));
   Bounds(2,:) = min(Bounds(2,:),min(pT,pT/pX * bP + Options(4)));
   if any(diff(Bounds < 0))
      error('The band is incompatible with the fixed boundaries'); end
...
Int_Index = Int_Coeff;
if ~Pred_Bound
   [A,B]                             = InterpCoeff(LenSeg(2,1) + 1,LenSeg(1,1) + Slacks_vec + 1,Slacks_vec); 
   [Int_Coeff{1:nSeg - 1}]           = deal(A);
   [Int_Index{1:nSeg - 1}]           = deal(B);
   [Int_Coeff{nSeg},Int_Index{nSeg}] = InterpCoeff(LenSeg(2,nSeg) + 1,LenSeg(1,nSeg) + Slacks_vec + 1,Slacks_vec);   
else
   for i_seg = 1:nSeg
      [Int_Coeff{i_seg},Int_Index{i_seg}] = InterpCoeff(LenSeg(2,i_seg) + 1,LenSeg(1,i_seg) + Slacks_vec + 1,Slacks_vec);
   end
end
...
for i_seg = 1:nSeg                             % Loop over segments
   ...
   a             = Slacks_vec + LenSeg(1,i_seg);               % a,b,c: auxiliary values that depend only on segment number and not node
   ...
   for i_node = Node_A:Node_Z                                  % Loop over nodes (i.e. possible boundary positions) for segment i_seg
      ...
      if N_AA % Sometimes boundaries are ineffective and few nodes are allowed that cannot be reached
         ...
         Ti_Seg                        = reshape((Ti_Seg + Coeff_b .* Ti_diff)',c,N_AA);                 % Interpolate for all allowed predecessors
...
if nargout > 1    % Reconstruct aligned signals
   ...
   for i_seg = 1:nSeg
      ...
      indX = bP(i_seg):bP(i_seg + 1);
      lenX = bP(i_seg + 1) - bP(i_seg);
      for i_sam = 1:nX
         indT                = Warping(i_sam,i_seg,2):Warping(i_sam,i_seg + 1,2);
         lenT                = Warping(i_sam,i_seg + 1,2) - Warping(i_sam,i_seg,2);
         % NB the right handside expression must be transposed to fit MATLAB version 6.5
         XWarped(i_sam,indT) = interp1q(indX' - Warping(i_sam,i_seg,1) + 1,X(i_sam,indX)',(0:lenT)'/lenT * lenX + 1)';
      end
