function [optim_pars,OS,diagnos] = optim_cow_orig_mw(X,optim_space,options,ref)
% function [optim_pars,OS,diagnos] = optim_cow_orig(X,optim_space,options,ref)
...
if (length(options) ~= 6)
    options_def                    = [0 3 50 0.15 5 5];
    options(length(options) + 1:6) = options_def(length(options) + 1:6);
end
...
dimX = size(X);
S    = sum(svd(reshape(X,dimX(1),numel(X)/dimX(1))/norm(reshape(X,dimX(1),numel(X)/dimX(1)),'fro')).^4);
ag   = unique(round(linspace(optim_space(1),optim_space(2),options(5))));
bg   = unique(round(linspace(optim_space(3),optim_space(4),options(6))));
...
for a=1:length(ag)
   ...
   for b=1:length(bg)
      ...
      if options(1)
        ...
        OS(2,N) = bg(b);
        OS(3:5,N) = [0 0 0];
        [OS(:,N),exitflag] = optim_eval(X,OS(1:2,N),OS,ref,round(length(ref)*options(4))); pause(5);
        N = N + 1;
        if options(1)
...
if (nargout > 2)
    ...
    try
        [diagnos.warping,diagnos.Xw,diagnos.warping_diagnos] = cow_orig_mw(ref,X,optim_pars(1),optim_pars(2),[0 1 0 round(length(ref)*options(4)) 0]);
    catch
...
if index3
    ...
    if (p(1) <= p(2)+3) || (p(2) < 1)% segment > slack OR slack < 1
        ...
        K = size(X,1);
        ind = repmat({':'},ndims(X) - 1,1);
        for k = 1:K
            normX(k) = norm(X(k,:));
        end
        try
            [warping,X,diagnos] = COW_orig_mw(ref,X,p(1),p(2),[0 1 0 losange 0]);
        catch
            for a=1:K
                [warping,X(a,:),diagnos] = COW_orig_mw(ref,X(a,ind{:}),p(1),p(2),[0 1 0 losange 0]);
            end
        end
        y(1) = diagnos.segment_length(1,1)+1;
        y(2) = diagnos.slack;
        t    = svd(reshape(X,dimX(1),numel(X)/dimX(1)) * reshape(X,dimX(1),numel(X)/dimX(1))');
        y(4) = sum(t.^2)/sum(t)^2;
        for k = 1:K
